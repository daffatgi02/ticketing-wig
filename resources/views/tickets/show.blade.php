                                    ...
                                    <div class="me-3 mb-3">
                                        <div class="card" style="width: 150px;">
                                            @if(in_array($attachment->filetype, ['image/jpeg', 'image/png', 'image/jpg', 'image/gif']))
                                                <img src="{{ Storage::url($attachment->filepath) }}" class="card-img-top" alt="{{ $attachment->filename }}" style="height: 100px; object-fit: cover;">
                                            @else
                                            ...
                                            <div class="card-body p-2">
                                                <p class="card-text" style="font-size: 0.8rem;">{{ Str::limit($attachment->filename, 15) }}</p>
                                                <a href="{{ Storage::url($attachment->filepath) }}" class="btn btn-sm btn-primary" target="_blank">View</a>
                                            </div>
                                        </div>
                                    </div>
                                @endforeach
            ...
            <div class="card mb-4">
                ...
                <div class="card-body">
                    @forelse($ticket->comments as $comment)
                        <div class="mb-4 p-3 {{ $comment->user_id == Auth::id() ? 'bg-light rounded' : 'border-start border-4 border-primary ps-3' }}">
                                ...
                                <div class="d-flex flex-wrap">
                                    @foreach($comment->attachments as $attachment)
                                        <div class="me-3 mb-3">
                                            <div class="card" style="width: 150px;">
                                                @if(in_array($attachment->filetype, ['image/jpeg', 'image/png', 'image/jpg', 'image/gif']))
                                                    <img src="{{ Storage::url($attachment->filepath) }}" class="card-img-top" alt="{{ $attachment->filename }}" style="height: 80px; object-fit: cover;">
                                                @else
                                                ...
                                                @endif
                                                <div class="card-body p-2">
                                                    <p class="card-text" style="font-size: 0.7rem;">{{ Str::limit($attachment->filename, 12) }}</p>
                                                    <a href="{{ Storage::url($attachment->filepath) }}" class="btn btn-sm btn-primary" target="_blank">View</a>
                                                </div>
                                            </div>
                                        </div>
                            ...
                            @if(Auth::user()->isAdmin() || $comment->user_id == Auth::id())
                                <div class="text-end">
                                    <form action="{{ route('ticket.comments.destroy', $comment) }}" method="POST" class="d-inline">
                                        @csrf
                                        @method('DELETE')
                    ...
                    @if(!in_array($ticket->status, ['closed', 'rejected']))
                        <div class="mt-4">
                            <h5>Add Comment</h5>
                            <form action="{{ route('ticket.comments.store', $ticket) }}" method="POST" enctype="multipart/form-data">
                                @csrf
                                <div class="mb-3">
        ...
        <div class="col-md-4">
            ...
            <div class="card mb-4">
                ...
                <div class="card-body">
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select @error('assigned_to') is-invalid @enderror" id="assigned_to" name="assigned_to" required>
                                    ...
                                    @foreach($supportStaff as $staff)
                                        <option value="{{ $staff->id }}">{{ $staff->name }} ({{ $staff->role == 'it_support' ? 'IT Support' : 'GA Support' }})</option>
                                    @endforeach
                                </select>
                                @error('assigned_to')
                                    <div class="invalid-feedback">{{ $message }}</div>
                    ...
                    @if(Auth::user()->isAdmin() || ($ticket->assigned_to == Auth::id() && in_array($ticket->status, ['assigned', 'in_progress'])))
